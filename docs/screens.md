...
| 基準サイズ・行間 | 本文16px / 1.7、設問文20〜22px相当 / 1.5 | **確定** |
...
- 5件法: 絶対にいや／いや／どちらでもない／好き／とても好き
- 選択すると次の問へ自動で進む。ココロパレアの挙動に合わせる
- 「前の質問」は `currentIndex === 0` のときのみ無効
...
- 通常回答と見直しの前問操作はともに「前の質問」。設問文字・5件法の選択肢・前問ボタンはココロパレアの既存回答画面と同じ寸法・文字組み・角丸を使い、色だけシゴトソケットの紺へ置き換える
- 回答画面と完了確認には、既定で閉じた「その他の操作」を置き、その中へ「回答を破棄」を表示する。確認後に削除するのは途中回答だけで、保存済み結果は残す
- ヘッダーはココロパレアと同じ文字組み・配置を使う。外枠、囲い面、角丸、影は付けず、下罫線だけで本文と区切る。開始・回答中・回答完了で同じ外枠classと余白を使い、回答中だけ固定表示や追加の上・左右余白を付けない
...
- 第2フェーズ: **「ココロパレアと合わせて見えたこと」1節に②③をまとめ、ホランド型の後・点数表の前に置く**。
  ...
  - ②の前置きで、ココロパレアは「性格の傾向」、シゴトソケットは「仕事でやってみたい活動」を示すものとして区別する
  - 各件は、内容と高め／低めが分かる見出し、ココロパレアの説明、シゴトソケットの説明、断定を避けた組み合わせの読み方、の順で表示する
  - 相関値は主本文へ混ぜず、各件の末尾に既定で閉じた「研究上の根拠を見る」を置く。開くと英語原版のrと、0／＋1／−1の読み方を表示する
- 免責の常時表示（F-008）
...
- 「テキストをコピー」は、先頭2行のアプリ名・アクセス先URLに続けて、称号・説明・ホランド型・連携結果・8尺度の点数・免責をコピーする
- アクセス先URLは現在のHTTPS URLを基に、結果IDを含むクエリ・ハッシュとファイル名を除いたアプリ入口URLとする。取得できない場合やローカルHTTPでは正式URL `https://sigotosocket.sikumilab.com/` を使う
- **共有対象は画像と結果テキストのみ。生回答・連携コードは含めない。URLだけを表示・コピーする領域も置かない**
