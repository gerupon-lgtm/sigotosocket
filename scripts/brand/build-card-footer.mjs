...
function footer(noteSize, innerGap, aboveGap) {
  const bandBottom = B("note1") - noteSize - 16;
  const band = { y: bandBottom - 100, h: 100 };
  const hollandBase = band.y - 22;
  const top2Base = hollandBase - innerGap;
  return { top: top2Base - 30 - aboveGap, band, parts: [
...
const variants = [
  { label: "A　塊として扱う（間隔42px・上を広く）", gap: 42, above: 60 },
  { label: "B　対等に離す（間隔70px・上は標準）", gap: 70, above: 32 },
];
const crops = [];
for (const v of variants) {
  const f = footer(15, v.gap, v.above);
  ...
  console.log(`${v.label}: キャラ＋レーダーに使えるのは y=360〜${f.top.toFixed(0)} の ${(f.top - 360).toFixed(0)}px`);
}
const cw = 760, ch = Math.round((H - 960) * 760 / W), gap = 24;
const SW = cw + gap * 2, SH = (ch + 44) * variants.length + gap;
const labels = variants.map((v, i) => `<text x="${gap}" y="${gap + i*(ch+44) + 24}" font-family="Noto Sans CJK JP" font-size="21" fill="#1b2a44">${v.label}</text>`).join("");
...
await sharp(base).composite(crops.map((b, i) => ({ input: b, left: gap, top: gap + i*(ch+44) + 36 }))).png().toFile("docs/brand/card/結論の塊.png");
console.log("ok");
